...
import com.fsnip.jg.modules.cms.entity.*;
...
import com.fsnip.jg.modules.cms.service.link.ILinkService;
...
public class FrontController extends BaseController {
	...
	public String index(@PathVariable String siteId, Model model) {
		...
		List<Category> mainNavList = null;
				//CmsUtils.getMainNavList(siteId);
		if (mainNavList.size() > 0){
			String firstCategoryId = null;
					//CmsUtils.getMainNavList(siteId).get(0).getId();
...
//			return "modules/cms/front/themes/"+category.getSite().getTheme()+"/"+getTpl(article);
	...
	public String map(@PathVariable String siteId, Model model) {
		Site site = null;
//		CmsUtils.getSite(siteId!=null?siteId: Site.defaultSiteId());
		model.addAttribute("site", site);
